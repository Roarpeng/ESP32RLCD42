...
private:
    ...
    // 开机配网跳过标志：USER 长按 3 秒时设为 true
    std::atomic<bool> boot_skip_requested_{false};
    ...
    void InitializeTools() {
        ...
        // ===== 相册 MCP 工具 =====
        mcp_server.AddTool("self.photo.switch",
            "Switch display to photo album mode and start slideshow.\n"
            "Use when user says: '切换到相册', '打开相册', '显示照片', 'show photos'",
            PropertyList(),
            [this](const PropertyList&) -> ReturnValue {
                if (display_) {
                    display_->NotifyUserActivity();
                ...
                pm.StartSlideshow();
                return std::string("已切换到相册模式，共") + std::to_string(pm.GetPhotoCount()) + "张图片";
            });

        mcp_server.AddTool("self.photo.next",
            "Show next photo in the slideshow.\n"
            "Use when user says: '下一张', '换一张', 'next photo'",
            PropertyList(),
            [](const PropertyList&) -> ReturnValue {
                PhotoManager::GetInstance().NextPhoto();
                return std::string("已切换到下一张");
            });

        mcp_server.AddTool("self.photo.prev",
            "Show previous photo in the slideshow.\n"
            "Use when user says: '上一张', '前一张', 'previous photo'",
            PropertyList(),
            [](const PropertyList&) -> ReturnValue {
                PhotoManager::GetInstance().PrevPhoto();
                return std::string("已切换到上一张");
            });

        mcp_server.AddTool("self.photo.interval",
            "Set the slideshow interval in seconds.\n"
            "Use when user says: '设置轮播间隔为30秒', '每5秒换一张', 'change to 10 seconds'\n"
            "Args:\n"
            "  `seconds`: interval between photos (3-300)",
            PropertyList({
                Property("seconds", kPropertyTypeInteger, 3, 300)
            }),
            [](const PropertyList& properties) -> ReturnValue {
                int sec = properties["seconds"].value<int>();
                auto& pm = PhotoManager::GetInstance();
                pm.GetConfig().interval = sec;
                pm.SaveConfig();
                pm.StopSlideshow();
                pm.StartSlideshow();
                return std::string("轮播间隔已设置为") + std::to_string(sec) + "秒";
            });
    }
...
public:
    CustomBoard() : boot_button_(BOOT_BUTTON_GPIO), user_button_(USER_BUTTON_GPIO) {    
        ...
        // 初始化相册管理器（HTTP 服务器将在 WiFi 连接后启动）
        auto& pm = PhotoManager::GetInstance();
        pm.Initialize("/sdcard/photos");
        ESP_LOGI(TAG, "相册管理器已初始化（%d 张图片），HTTP 服务器等待 WiFi 就绪", pm.GetPhotoCount());
