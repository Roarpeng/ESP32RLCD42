...
void CustomLcdDisplay::DataUpdateTask(void *arg) {
    ...
    // 联网后是否已刷新过相册桌面（更新上传 URL 为真实 IP）
    bool post_connect_photo_updated = false;
    ...
    while (1) {
        ...
        // ===== 联网后刷新相册桌面（用真实 IP 更新上传 URL）=====
        if (network_connected && !post_connect_photo_updated) {
            post_connect_photo_updated = true;
            self->UpdatePhotoDesktopStatus();
            ESP_LOGI(TAG, "联网后已刷新相册桌面上传地址");
        }
        
        // ===== 天气更新 =====
        if (network_connected && idle_long_enough) {
            static uint32_t weather_last_sync_ms = 0;
            static bool weather_synced = false;
            if (!weather_synced || (now_ms - weather_last_sync_ms > 4 * 3600 * 1000)) { // 4小时更新一次
                ESP_LOGI(TAG, "更新天气信息...");
        ...
        if (!self->showing_system_info_) {
            ...
            if (minute_changed) {
                ...
                if (self->clock_date_label_) {
                    char clock_date_buf[24];
                    snprintf(clock_date_buf, sizeof(clock_date_buf), "%02d/%02d %s",
                             timeinfo.tm_mon + 1, timeinfo.tm_mday, weeks_title[timeinfo.tm_wday]);
                    lv_label_set_text(self->clock_date_label_, clock_date_buf);
                }
            ...
            if (allow_noncritical_update) {
                ...
                if (sd.valid) {
                    if (fabs(sd.temperature - self->last_temp_) > 0.2f || fabs(sd.humidity - self->last_humi_) > 1.0f) {
                        ...
                        if (self->quote_sensor_label_) {
                            char temp_buf[16];
                            snprintf(temp_buf, sizeof(temp_buf), "%.1f°C", sd.temperature);
                            lv_label_set_text(self->quote_sensor_label_, temp_buf);
                        }
                        if (self->clock_sensor_label_) {
                            char temp_buf[16];
                            snprintf(temp_buf, sizeof(temp_buf), "%.1f°C", sd.temperature);
                            lv_label_set_text(self->clock_sensor_label_, temp_buf);
                        }
                        if (self->clock_temp_label_) {
                            char temp_buf[16];
                            snprintf(temp_buf, sizeof(temp_buf), "%.1f°C", sd.temperature);
                            lv_label_set_text(self->clock_temp_label_, temp_buf);
                        }
                        if (self->weather_humidity_label_) {
                            char humi_buf[12];
                            snprintf(humi_buf, sizeof(humi_buf), "%.0f%%", sd.humidity);
                            lv_label_set_text(self->weather_humidity_label_, humi_buf);
                        }
                ...
                if (wd.valid && self->weather_label_) {
                    ...
                    if (last_weather_text != weather_buf) {
                        ...
                        if (self->weather_temp_big_label_) {
                            char temp_big[16];
                            snprintf(temp_big, sizeof(temp_big), "%s°C", wd.temp.c_str());
                            lv_label_set_text(self->weather_temp_big_label_, temp_big);
                        }
                        if (self->weather_realfeel_label_) {
                            char real_buf[16];
                            snprintf(real_buf, sizeof(real_buf), "%s°C", wd.temp.c_str());
                            lv_label_set_text(self->weather_realfeel_label_, real_buf);
                        }
                ...
                if (battery_cached) {
                    ...
                    if (icon_mode != last_icon_mode) {
                        ...
                        if (self->quote_battery_icon_img_) {
                            lv_image_set_src(self->quote_battery_icon_img_, icon_src);
                        }
                        if (self->photo_battery_icon_img_) {
                            lv_image_set_src(self->photo_battery_icon_img_, icon_src);
                        }
                        if (self->clock_battery_icon_img_) {
                            lv_image_set_src(self->clock_battery_icon_img_, icon_src);
                        }
                    ...
                    if (cached_battery_level != last_battery_level) {
                        ...
                        if (self->battery_pct_label_) lv_label_set_text(self->battery_pct_label_, bat_buf);
                        ...
                        if (self->quote_battery_pct_label_) lv_label_set_text(self->quote_battery_pct_label_, bat_buf);
                        if (self->photo_battery_pct_label_) lv_label_set_text(self->photo_battery_pct_label_, bat_buf);
                        if (self->clock_battery_pct_label_) lv_label_set_text(self->clock_battery_pct_label_, bat_buf);
                ...
                if (ds != last_wifi_state) {
                    ...
                    if (self->quote_wifi_icon_img_) {
                        lv_image_set_src(self->quote_wifi_icon_img_, wifi_src);
                    }
                    if (self->photo_wifi_icon_img_) {
                        lv_image_set_src(self->photo_wifi_icon_img_, wifi_src);
                    }
                    if (self->clock_wifi_icon_img_) {
                        lv_image_set_src(self->clock_wifi_icon_img_, wifi_src);
                    }
                    last_wifi_state = ds;
                }

                // 6. 相册状态更新（照片数量或索引变化时更新UI）
                static int last_photo_count = -1;
                static int last_photo_index = -1;
                int current_photo_count = PhotoManager::GetInstance().GetPhotoCount();
                int current_photo_index = PhotoManager::GetInstance().GetCurrentIndex();
                
                if (current_photo_count != last_photo_count) {
                    self->UpdatePhotoDesktopStatus();
                    if (last_photo_count <= 0 && current_photo_count > 0) {
                        PhotoManager::GetInstance().StartSlideshow();
                    }
                    last_photo_count = current_photo_count;
                }
                
                if (current_photo_index != last_photo_index) {
                    self->UpdatePhotoImage();
                    self->UpdatePhotoDesktopStatus();
                    last_photo_index = current_photo_index;
                }
            }
