...
void WifiBoard::OnNetworkEvent(NetworkEvent event, const std::string& data) {
    switch (event) {
        case NetworkEvent::Connected:
            // Stop timeout timer
            esp_timer_stop(connect_timer_);
            GetDisplay()->HideWifiProvisioningQr();
#ifdef CONFIG_USE_ESP_BLUFI_WIFI_PROVISIONING
...
void WifiBoard::StartWifiConfigMode() {
    ...
    Application::GetInstance().Schedule([&wifi_manager]() {
        ...
        std::string hint = "请扫描屏幕二维码连接设备热点并打开配网页\nSSID: ";
        hint += ap_ssid;
        hint += "\nURL: ";
        hint += ap_url;

        Application::GetInstance().Alert("扫码配网", hint.c_str(), "gear", Lang::Sounds::OGG_WIFICONFIG);
        Board::GetInstance().GetDisplay()->ShowWifiProvisioningQr(ap_ssid.c_str(), ap_url.c_str());
    });
