...
#include "assets.h"
#include "settings.h"
#ifdef CONFIG_BOARD_TYPE_WAVESHARE_S3_RLCD_4_2
...
void Application::HandleNetworkConnectedEvent() {
    ...
    // Start photo album HTTP server now that LWIP is ready
#ifdef CONFIG_BOARD_TYPE_WAVESHARE_S3_RLCD_4_2
    PhotoHttpServer::GetInstance().Start(80);
#endif

    if (state == kDeviceStateStarting || state == kDeviceStateWifiConfiguring) {
        // 恢复正常的激活流程：OTA 检查 + 协议初始化（建立 AI 通道）
        // AI 通道建立后可自动获取天气和格言
        ...
        if (activation_task_handle_ == nullptr) {
            xTaskCreate([](void* arg) {
                ...
                app->ActivationTask();
                vTaskDelete(NULL);
            }, "activation", 8192 * 2, this, 5, &activation_task_handle_);
        }
    }
...
void Application::HandleActivationDoneEvent() {
    ...
    // PlaySound disabled — audio stubs don't contain valid OGG data
...
void Application::ResetProtocol() {
    StopMusicPlayback();
#ifdef CONFIG_BOARD_TYPE_WAVESHARE_S3_RLCD_4_2
    PhotoManager::GetInstance().StopSlideshow();
#endif
...
/* Audio embed stubs — inline asm to guarantee _start/_end surround exactly 64 zero bytes */
#define AUDIO_STUB(name) \
    __attribute__((used, aligned(4))) uint8_t _binary_##name##_buf[64] = {0}; \
    asm volatile( \
        ".global _binary_" #name "_start\n" \
        ".global _binary_" #name "_end\n" \
        "_binary_" #name "_start = _binary_" #name "_buf\n" \
        "_binary_" #name "_end = _binary_" #name "_buf + 64\n" \
    )
...
AUDIO_STUB(8_ogg);
AUDIO_STUB(9_ogg);
AUDIO_STUB(activation_ogg);
AUDIO_STUB(err_pin_ogg);
AUDIO_STUB(err_reg_ogg);
AUDIO_STUB(exclamation_ogg);
AUDIO_STUB(low_battery_ogg);
AUDIO_STUB(popup_ogg);
AUDIO_STUB(success_ogg);
AUDIO_STUB(upgrade_ogg);
AUDIO_STUB(vibration_ogg);
AUDIO_STUB(wificonfig_ogg);
}
